...
Amazon SQS REST Wrapper

Credits:

Derived from https://github.com/coldbox-modules/snssdk created by Luis Majano ( Ortus Solutions )
...
//------------------------------------------- PUBLIC ------------------------------------------>
	...
	* 
	*/ 
	function sendMessage( required accountid, required string queueName, required messageBody){
		// Invoke call
		var parameters = {
			"Action":"SendMessage",
			"MessageBody": messageBody
		}
		for(key in structKeyList(arguments)){
			if(not listFindNoCase("accountid,queuename,messageBody",key)){
				parameters[key] = arguments[key];
			}
		}
		var results = awsRequest(
			resource="#arguments.accountid#/#arguments.queueName#",
	...
	function receiveMessage( required accountid, required string queueName){
		// Invoke call
		var parameters = {
			"Action":"ReceiveMessage",
			"MaxNumberOfMessages": arguments.maxNumberOfMessages
		}
		for(key in structKeyList(arguments)){
			if(not listFindNoCase("accountid,queuename,messageBody",key)){
				parameters[key] = arguments[key];
	...
	*/
    private struct function awsRequest(
		...
		if( structKeyExists( HTTPResults.responseHeader, "content-type" ) AND
		    (HTTPResults.responseHeader["content-type"] eq "application/xml" or HTTPResults.responseHeader["content-type"] eq "text/xml") AND
			...
			if( NOT listFindNoCase( "200,204", HTTPResults.responseHeader.status_code ) ){
				// check error xml
				results.error 	= true;
				results.message = "Code: #results.response.ErrorResponse.error.code.XMLText#. Message: #results.response.ErrorResponse.error.message.XMLText#";
			}
		}
